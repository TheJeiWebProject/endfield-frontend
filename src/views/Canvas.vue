<template>
  <div class="canvas-page">
    <canvas id="canvasBgCanvas" class="page-bg-canvas"></canvas>

    <div class="drawer-overlay" :class="{ visible: showLeftDrawer || showRightDrawer }" @click="closeAllDrawers"></div>

    <aside class="drawer drawer-left" :class="{ open: showLeftDrawer }">
      <div class="drawer-content">
        <div class="drawer-header">
          <h3>{{ i18n.t('items.title') }}</h3>
          <button class="drawer-close-btn" @click="showLeftDrawer = false">X</button>
        </div>

        <div class="search-box">
          <input v-model="itemSearch" type="text" :placeholder="i18n.t('canvas.search')">
        </div>

        <div class="continuous-place-mode">
          <label class="checkbox-label">
            <input v-model="continuousPlaceMode" type="checkbox">
        ...
        <div class="layer-controls-sidebar">
          ...
          <div class="layer-buttons">
            <button class="layer-btn-small" :class="{ active: activeLayer === 'all' }" @click="setActiveLayer('all')">
              {{ i18n.t('canvas.layer.all') }}
            </button>
            <button class="layer-btn-small" :class="{ active: activeLayer === 'layer1' }" @click="setActiveLayer('layer1')">
              {{ i18n.t('canvas.layer.belt') }}
            </button>
            <button class="layer-btn-small" :class="{ active: activeLayer === 'layer2' }" @click="setActiveLayer('layer2')">
              {{ i18n.t('canvas.layer.pipe') }}
            </button>
          </div>
          <div class="layer-checkboxes">
            <label class="checkbox-label">
              <input type="checkbox" :checked="showLayer1" @change="setShowLayer1($event.target.checked)">
              <span>{{ i18n.t('canvas.layer1.visible') }}</span>
            </label>
            <label class="checkbox-label">
              <input type="checkbox" :checked="showLayer2" @change="setShowLayer2($event.target.checked)">
        ...
        <div class="category-tabs">
          <button v-for="cat in displayCategories" :key="cat.id"
        ...
        <div class="item-palette-wrapper">
          <div class="item-palette">
            <div v-for="item in filteredItems" :key="item.id"
              class="palette-item" :class="{ selected: selectedPaletteItem?.id === item.id }"
              draggable="true"
              @dragstart="handleDragStart($event, item)"
              @click="selectPaletteItem(item)">
              <img :src="dataStore.getIconUrl(item.icon)" :alt="i18n.t('item.' + item.id)">
    ...
    <aside class="drawer drawer-right" :class="{ open: showRightDrawer }">
      <div class="drawer-content drawer-right-inner">
        <div class="drawer-header">
          <h3>Control Panel</h3>
          <button class="drawer-close-btn" @click="showRightDrawer = false">X</button>
        </div>
        <RightSidebar
          :active-tab="rightTab"
    ...
    <div class="canvas-wrapper">
      <canvas id="gameCanvas" ref="canvasRef" tabindex="0"
    ...
    <button class="drawer-toggle drawer-toggle-left"
    ...
    <button class="drawer-toggle drawer-toggle-right"
    ...
    <div class="status-bar">
      <span class="status-section">
        <span class="status-tool">{{ getToolLabel(currentTool) }}</span>
      </span>
      <span class="status-sep">|</span>
      <span class="status-section">
        <span class="status-region">{{ regionLabel }}</span>
        <span v-if="isWulingRegion" class="status-layer"> · {{ layerLabel }}</span>
      </span>
      <span class="status-sep">|</span>
      <span class="status-section">
        <span class="status-label">缩放</span>
        <span class="status-value">{{ Math.round(zoom * 100) }}%</span>
      </span>
      <span class="status-sep">|</span>
      <span class="status-section">
        <span class="status-label">物品</span>
        <span class="status-value">{{ placedCount }}</span>
      </span>
      <span class="status-sep">|</span>
      <span class="status-section">
        <span class="status-label">传送带</span>
        <span class="status-value">{{ beltCount }}</span>
        <span class="status-label">管</span>
        <span class="status-value">{{ pipeCount }}</span>
      </span>
      <span class="status-sep">|</span>
      <span class="status-section">
        <span class="status-label">电力</span>
        <span class="status-value">{{ totalPower }}kW</span>
      </span>
    </div>
    ...
    <BlueprintPanel v-if="showBlueprintPanel"
      :blueprints="blueprints"
    ...
    <RecipePanel v-if="showRecipePanel"
      :selected-item="selectedItem"
...
const openTutorial = inject('openTutorial', () => {})
...
const regionLabel = computed(() => {
  ...
  return regionMap[selectedRegion.value] || selectedRegion.value || '四号谷地 · 枢纽区'
})

const layerLabel = computed(() => {
  const map = { all: '全部', layer1: '传送带层', layer2: '管道层' }
  return map[activeLayer.value] || '全部'
})

function getToolLabel(tool) {
  const map = { select: '选择', place: '放置', belt: '传送带', pipe: '管道', delete: '删除' }
...
function selectRecipe(recipe) {
  ...
  if (recipe && recipe.out) {
    ...
    if (outputId) {
      const producerIds = recipe.producers || []
      
      if (producerIds.length > 0) {
        const machineItem = dataStore.items.find(item => 
          producerIds.includes(item.id) && item.category === 'machine'
        )
        
        if (machineItem && canvasRef.value && gameCanvas.value) {
          // 自动放置：画布中心位置
          ...
          // 使用 nextTick 确保 DOM 更新后再放置
          setTimeout(() => {
            const placed = gameCanvas.value?.placeItemAt(machineItem, centerX, centerY)
...
function handleKeyDown(e) {
  if (e.key === 'Escape') {
    if (showLeftDrawer.value || showRightDrawer.value) {
      closeAllDrawers()
      return
    }
...
<style scoped>
/* Panel 1: Full Canvas + Drawer Panels */
.canvas-page {
  position: fixed;
  top: 40px;
  left: 0;
...
.drawer-right-inner { padding: 0; }
.drawer-left .drawer-content { padding: 8px 12px; }
...
/* Continuous place */
.continuous-place-mode { margin-bottom: 8px; }
...
.layer-buttons { display: flex; gap: 4px; margin-bottom: 6px; }
.layer-btn-small {
  flex: 1;
  padding: 3px 6px;
  border: 1px solid rgba(0, 212, 255, 0.25);
...
.layer-btn-small:hover { background: rgba(0, 212, 255, 0.12); }
.layer-btn-small.active {
  background: #00D4FF;
  border-color: #00D4FF;
  color: #040810;
}
.layer-checkboxes { display: flex; flex-direction: column; gap: 3px; }
.layer-checkboxes .checkbox-label { font-size: 10px; color: #888; }
...
/* Categories */
.category-tabs { display: flex; gap: 4px; margin-bottom: 8px; flex-wrap: wrap; }
.category-tab {
  padding: 4px 8px;
  border: 1px solid rgba(0, 212, 255, 0.25);
  border-radius: 3px;
...
.category-tab:hover { background: rgba(0, 212, 255, 0.12); }
.category-tab.active { background: #00D4FF; color: #040810; }

/* Item palette */
.item-palette-wrapper { flex: 1; overflow-y: auto; }
.item-palette { display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; }
.palette-item {
  display: flex;
  flex-direction: column;
  align-items: center;
...
.palette-item:hover { background: rgba(0, 212, 255, 0.12); border-color: rgba(0, 212, 255, 0.4); }
.palette-item.selected { background: rgba(0, 212, 255, 0.18); border-color: #00D4FF; }
.palette-item img { width: 28px; height: 28px; object-fit: contain; }
.palette-item span { margin-top: 3px; font-size: 10px; color: #eaeaea; text-align: center; }
...
.drawer-toggle:hover { background: rgba(0, 212, 255, 0.12); border-color: rgba(0, 212, 255, 0.5); }
.drawer-toggle.active { background: rgba(0, 212, 255, 0.15); border-color: #00D4FF; }
.toggle-icon { color: #7A8FA6; font-size: 14px; transition: color 0.2s; }
.drawer-toggle:hover .toggle-icon,
.drawer-toggle.active .toggle-icon { color: #00D4FF; }
.drawer-toggle-left { left: 0; border-radius: 0 4px 4px 0; }
.drawer-toggle-right { right: 0; border-radius: 4px 0 0 4px; }
...
.status-sep { color: rgba(0, 212, 255, 0.15); font-size: 11px; }


/* Scrollbars */
.drawer-content::-webkit-scrollbar { width: 5px; }
.drawer-content::-webkit-scrollbar-track { background: rgba(10, 10, 10, 0.3); }
.drawer-content::-webkit-scrollbar-thumb { background: rgba(0, 212, 255, 0.4); border-radius: 3px; }
.drawer-content::-webkit-scrollbar-thumb:hover { background: rgba(0, 212, 255, 0.6); }
.item-palette-wrapper::-webkit-scrollbar { width: 4px; }
.item-palette-wrapper::-webkit-scrollbar-track { background: transparent; }
.item-palette-wrapper::-webkit-scrollbar-thumb { background: rgba(0, 212, 255, 0.3); border-radius: 2px; }

/* Responsive */
@media (max-width: 900px) {
  .drawer-left { width: 220px; }
  .drawer-right { width: 260px; }
  .drawer-toggle { width: 20px; height: 48px; }
}
</style>
